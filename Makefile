...
export MTD_LOADER_LENGTH := $(CONFIG_MTD_BLOCK_UBOOT_LENGTH)
export MTD_KERNEL_LENGTH := $(CONFIG_MTD_BLOCK_KERNEL_LENGTH)
export MTD_ROOTFS_LENGTH := $(CONFIG_MTD_BLOCK_ROOTFS_LENGTH)
export MTD_MAC_LENGTH    := $(CONFIG_MTD_BLOCK_MAC_LENGTH)

ifeq ($(CONFIG_LP_SUPPORT),y)
export MTD_LP_LENGTH	 := $(CONFIG_MTD_BLOCK_LP_LENGTH)
endif

export MTD_ART_LENGTH    := $(CONFIG_MTD_BLOCK_ART_LENGTH)
...
LIBS += common/libcommon.a
# What was there in D-Link: LIBS += httpd/libhttpd.a
LIBS += $(BOARDLIBS)
.PHONY : $(LIBS)
...
ALL = config u-boot_env u-boot.srec u-boot.bin System.map

all:		$(ALL)
config:
	cp -f $(PROJECTS_PATH)/include/defaultConf.h ./httpd
	cp -f $(PROJECTS_PATH)/include/bsp.h     ./httpd


u-boot_env	:
		echo "" > include/configs/cameo.h
		echo "#define MTD_LOADER_LENGTH \"$(MTD_LOADER_LENGTH)\"" >> include/configs/cameo.h
		echo "#define MTD_KERNEL_LENGTH \"$(MTD_KERNEL_LENGTH)\"" >> include/configs/cameo.h
		echo "#define MTD_ROOTFS_LENGTH \"$(MTD_ROOTFS_LENGTH)\"" >> include/configs/cameo.h
		echo "#define MTD_MAC_LENGTH \"$(MTD_MAC_LENGTH)\"" >> include/configs/cameo.h
		echo "#define MTD_ART_LENGTH \"$(MTD_ART_LENGTH)\"" >> include/configs/cameo.h
ifeq ($(CONFIG_LP_SUPPORT),y)
	echo "#define MTD_LP_LENGTH \"$(MTD_LP_LENGTH)\"" >> include/configs/cameo.h
	echo "#define LP_SUPPORT" >> include/configs/cameo.h
	cp -rf httpd/fsdata_lp.c httpd/fsdata.c
else	
	cp -rf httpd/fsdata_nml httpd/fsdata.c
endif
...
u-boot.bin:	u-boot
		...
		echo -n $(HARDWARE_ID) >> u-boot.bin
		cp -f u-boot.bin $(PROJECTS_IMAGE_PATH)/ldr.bin 
